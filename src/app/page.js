...
export default function Page() {
  ...
  return (
    <div class="flex flex-col w-full">
      ...
      <div class="flex justify-between w-full sm:max-md:flex-wrap md:h-28 bg-black">
      ...
      <div class="container min-h-screen h-auto bg-black w-full flex">
      <div class=" pt-1 px-4 md:p-4 h-auto w-auto flex-row">
       <form class="md:py-28 py-32">
        {inputs.map((input, i) => (
        <div key={i} class="md:pl-20 pl-4">
          <h1 class="text-4xl text-white text-opacity-100 py-2"><span className={jackquard.className}>Subject : {i+1}</span></h1>
          ...
          <div class="flex md:flex-row md:pt-2 md:pl-20 gap-3">
          {inputs.length - 1 === i && (
          ...
          {inputs.length !== 1 && (
            <button type="button" class="font-serif text-white bg-red-900  hover:bg-red-700 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-full text-sm px-5  me-2 mb-2  md:py-1 " onClick={() => handleRemoveClick(i)}>Remove</button>
          )}
          </div>
        </div>
       </div>
        ...
        <div class="pt-8 pl-4 md:pt-16 md:pl-20 flex gap-6 md:gap-10">
      ...
      <span class="relative font-serif" onClick={() => handleResetClick()}>Reset</span>
        ...
        {totalCredits!=0 && showRes && <h1 class="text-4xl md:text-5xl text-white text-opacity-100 py-10 pl-3 md:p-40 md:py-16"><span className={jackquard2.className}>Current SGPA :{(numCredits/totalCredits) * 10}</span></h1>}
      </form>
    </div>
